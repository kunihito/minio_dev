...
import 'package:minio_new/src/minio_custom_config.dart';
...
String getCanonicalRequest(
  ...
  final headers = signedHeaders.map(
        (header) {
          ...
          if (config.signedRequestHostnameConversionRules.containsKey(request.headers[header])) {
            return '${header.toLowerCase()}:${config.signedRequestHostnameConversionRules[request.headers[header]]}';
          } else {
            return '${header.toLowerCase()}:${request.headers[header]}';
          }
        },
  );
